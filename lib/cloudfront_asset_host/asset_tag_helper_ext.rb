    ...
    private
      ...
      def rails_asset_id_with_cloudfront(source)
        if @@cache_asset_timestamps && (asset_id = @@asset_timestamps_cache[source])
          puts "rails_asset_id_with_cloudfront(#{source}): cached asset_id #{asset_id}"
          asset_id
        else
          ...
          puts "rails_asset_id_with_cloudfront(#{source}): asset_id #{asset_id}"
      ...
      def rewrite_asset_path_with_cloudfront(source, path=nil)
        ...
        puts "rewrite_asset_path_with_cloudfront: source: #{source}, asset_id #{asset_id}"
        if asset_id.blank?
          source
        else
          "/#{asset_id}#{source}"
